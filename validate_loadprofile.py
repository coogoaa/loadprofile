...
def generate_md_report(calc, r, address="") -> str:
    ...
    ln("---")
    ln(f"*Generated by validate_loadprofile.py — {datetime.now().strftime('%Y-%m-%d %H:%M')}*")
    return "\n".join(lines)


# ── 目录批量运行 ───────────────────────────────────────────────────────────────
def run_input_dir(params, input_dir, output_dir):
    """读取 input_dir 中所有 .txt/.md 文件，逐一计算，MD 报告写入 output_dir。"""
    ...
    if not files:
        print(red(f"  ⚠  {input_dir} 中未找到 .txt / .md 文件"))
        return

    print(f"\n  📂 输入目录: {input_dir}  ({len(files)} 个文件)")
    print(f"  📁 输出目录: {output_dir}\n")

    summary = []
    for fp in files:
        print(f"  {dim('处理: ' + fp.name)}")
        try:
            kw, address = parse_input_file(str(fp))
            calc = LoadProfileCalculator(params, verbose=False, **kw)
            r    = calc.run()
            md   = generate_md_report(calc, r, address)
            out  = output_dir / (fp.stem + "_report.md")
            out.write_text(md, encoding="utf-8")
            summary.append((fp.stem, kw["state"], r))
            print(f"    {green('✅')} 已保存 → {out.name}")
    ...
    print(bold("  批量汇总"))
    print(f"{'═'*W}")
    print(f"  {'文件':<28} {'州':<5} {'年总 kWh':>10} {'白天%':>8} {'晚峰%':>8} {'整夜%':>8}")
    print(f"  {'─'*W}")
    for name, state, r in summary:
        print(f"  {name:<28} {state:<5} {r['final_annual']:>10,.0f}"
              f" {r['daytime_pct']:>8.2f}% {r['evening_peak_pct']:>8.2f}% {r['overnight_pct']:>8.2f}%")
    print(f"  {'─'*W}")
    print(f"{'═'*W}")
...
示例:
  ...
  python validate_loadprofile.py --input-dir input_dir --output-dir output_dir
        """
    )
    ap.add_argument("--input",       type=str,                   help="TXT/MD 输入文件（地址+5道题）")
    ap.add_argument("--input-dir",   type=str,                   dest="input_dir", help="输入目录，批量处理目录内所有 .txt/.md 文件")
    ap.add_argument("--output-dir",  type=str,                   dest="output_dir", default="outputs", help="MD 报告输出目录（默认: outputs/）")
    ap.add_argument("--validate",    action="store_true",        help="运行 PRD 标准示例并校验期望値")
    ...
    if args.input_dir:
        run_input_dir(params, args.input_dir, args.output_dir)
    elif args.input:
        kw, address = parse_input_file(args.input)
        print(f"  {dim('地址: ' + address + '  →  州: ' + kw['state'])}") if address else None
        calc = LoadProfileCalculator(params, verbose=True, **kw)
        r    = calc.run()
        md   = generate_md_report(calc, r, address)
        out  = Path(args.output_dir)
        out.mkdir(parents=True, exist_ok=True)
        stem = Path(args.input).stem
        rpt  = out / (stem + "_report.md")
        rpt.write_text(md, encoding="utf-8")
        print(f"\n  {green('✅')} MD 报告已保存 → {rpt}")
    elif args.validate:
        run_validate(params)
    elif args.batch:
