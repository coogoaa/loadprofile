# 系统组成报告（N 全新建） · case `11169` · tier `A`
...
|---|---|---|
| 州 | `BY` | request.json |
| SAM3D 满铺 | 101.05 kWp | auto_from_panel_location |
| 屋顶 2D 面积 | 130.84 m² | detect_building.json |
...
roof_capped       = min(target_pv_capped, SAM3D)
                  = min(7.05, 101.05) = 7.05 kWp
✓ 屋顶足够

panels_floor = floor(7.05 / 0.470) = 15
panels_ceil  = ceil(7.05 / 0.470)  = 15
  → 取 ceil 时是否破屋顶? ceil×0.470=7.050 ≤ SAM3D(101.05)
  → 选 panels = 15 块
pv_pre      = panels × 0.470 = 7.05 kWp
assert pv_pre ≤ min(25.0, SAM3D=101.05) → ✅
```
