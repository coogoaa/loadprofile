# 系统组成报告（N 全新建） · case `11173` · tier `B`
...
target_pv_capped  = min(target_pv_total, 25.0) = 10.34 kWp
roof_capped       = min(target_pv_capped, SAM3D)
                  = min(10.34, 9.40) = 9.40 kWp
⚠ 屋顶受限：SAM3D (9.40) < target (10.34)
...
bat_kWh       = ceil_to_spec(max(5, 8.46), [5, 6.5, 9.6, 10, 13.5, 16, 20, 25, 30, 35, 40, 45, 50]) = 9.6 kWh
```
