# 系统组成报告（N 全新建） · case `11176` · tier `C`
...
|---|---|---|
| 州 | `BY` | request.json |
| SAM3D 满铺 | 39.48 kWp | auto_from_panel_location |
| 屋顶 2D 面积 | 299.01 m² | detect_building.json |
...
roof_capped       = min(target_pv_capped, SAM3D)
                  = min(13.16, 39.48) = 13.16 kWp
✓ 屋顶足够

panels = floor(13.16 / 0.470) = 28 块
pv_pre = panels × 0.470 = 13.16 kWp
assert pv_pre ≤ min(25.0, SAM3D=39.48) → ✅
```
...
bat_target    = actual_pv × ratio = 13.16 × 1.2 = 15.79 kWh
bat_kWh       = ceil_to_spec(max(5, 15.79), [5, 6.5, 9.6, 10, 13.5, 16, 20, 25, 30, 35, 40, 45, 50]) = 16 kWh
```

## 5. 选板（按年发电量降序选前 N 块）

- actual_panels = 28 块
- 选中面板年发电量合计 = 13,406.1 kWh/年
...
| 电池 | 16 kWh (target 15.79, ratio 1.2) |
