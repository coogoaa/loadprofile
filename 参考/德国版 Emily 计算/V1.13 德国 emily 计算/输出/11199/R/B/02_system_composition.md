...
| 屋顶 2D 面积 (mask2d) | 293.23 m² | detect_building.json data.area |
...
```
cos(40°)            = 0.7660
roof_area_m2        = mask2d / cos(40°)         = 293.23 / 0.7660 = 382.78 m²
usable_area_m2      = roof_area_m2 × 0.45        = 382.78 × 0.45 = 172.25 m²
max_panels_area     = floor(172.25 / 2.1580) = 79 块
roof_full_kwp_area  = 79 × 0.47 = 37.13 kWp  (2D 估算)
roof_full_kwp_3d    = SAM3D = 11.28 kWp
```
...
remaining = max(0, roof_full_kwp_area - existing) = max(0, 37.13 - 22.00) = 15.13 kWp
remaining_capped = max(0, min(remaining, 25.0 - existing))
                = max(0, min(15.13, 25.0 - 22.00))
                = 3.00 kWp
```
...
- 选中面板年发电量合计 = 9,808.3 kWh/年（来自 panel_location.json `monthlyHourlyPowerList`）
- 实装 PV = 22.00 kWp
