...
|---|---|---|
| 州 | `BY` | request.json |
| 既有 PV (existing_input) | 7.00 kWp (已知) | cases.md Q1=5-10 |
| SAM3D 满铺 | 36.66 kWp | auto_from_panel_location |
| 屋顶 2D 面积 (mask2d) | 281.96 m² | detect_building.json data.area |
...
```
cos(40°)            = 0.7660
roof_area_m2        = mask2d / cos(40°)         = 281.96 / 0.7660 = 368.07 m²
usable_area_m2      = roof_area_m2 × 0.45        = 368.07 × 0.45 = 165.63 m²
max_panels_area     = floor(165.63 / 2.1580) = 76 块
roof_full_kwp_area  = 76 × 0.47 = 35.72 kWp  (2D 估算)
roof_full_kwp_3d    = SAM3D = 36.66 kWp
```
...
```
roof_full_kwp_3d - existing = 36.66 - 7.00 = 29.66 ≥ 0.47
remaining = max(0, roof_full_kwp_3d - existing) = 29.66 kWp
remaining_capped = max(0, min(remaining, 25.0 - existing))
                = max(0, min(29.66, 25.0 - 7.00))
                = 18.00 kWp
```
...
- 选中面板年发电量合计 = 12,360.0 kWh/年（来自 panel_location.json `monthlyHourlyPowerList`）
- 实装 PV = 13.11 kWp
